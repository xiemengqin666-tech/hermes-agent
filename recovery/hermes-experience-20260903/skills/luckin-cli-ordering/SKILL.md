...
- After loading this skill, do not browse the web, inspect product images, load unrelated skills, or write a manual skill-usage record. The Luckin CLI preview is the authoritative live source for menu attributes, coupons and price.
...
Target latency: a clear supported request should reach preview in about 30 seconds; confirmation should reach the payment QR in about 20 seconds under normal network conditions.

## Fast durable path

For `葡萄冰茶超大杯不加糖`, the model should interpret the request, then run one command only. It performs the live product check and exact preview together and persists the accepted tuple for 15 minutes:

```bash
python3 ~/.hermes/skills/productivity/luckin-cli-ordering/scripts/prepare_luckin_order_fast.py \
  --preset grape-ice-tea-xl-no-sugar \
  --target 'weixin:<origin_chat_id>'
```
...
```bash
python3 ~/.hermes/skills/productivity/luckin-cli-ordering/scripts/confirm_luckin_order_fast.py \
  --target 'weixin:<origin_chat_id>'
```
...
## 2. Resolve exact product and SKU (fallback for other drinks)
...
## 3. Preview once (fallback for other drinks)
